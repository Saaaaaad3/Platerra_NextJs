...
function ScrollRow({
  ...
  return (
    <div className="relative -mx-6">
      ...
      {canScrollLeft && (
        <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 left-0 w-10 bg-gradient-to-r from-brand-surface to-transparent" />
      )}
      {canScrollRight && (
        <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 right-0 w-10 bg-gradient-to-l from-brand-surface to-transparent" />
      )}
    </div>
  );
}
  ...
  return (
    <>
      ...
      >
        <div
          ref={modalRef}
          className="relative flex max-h-[92vh] w-full flex-col overflow-y-auto rounded-t-[2rem] bg-brand-surface text-brand-on-surface shadow-2xl animate-modal-panel sm:max-w-lg sm:rounded-[2rem]"
        ...
        >
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-brand-surface/90 text-brand-on-surface shadow-md transition hover:bg-brand-surface"
          >
          ...
          {/* Hero image */}
          <div
            className="relative h-72 w-full flex-shrink-0 touch-pan-y bg-brand-on-surface/10 sm:h-96"
          ...
          <div className="flex flex-col gap-6 px-6 py-8">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-start justify-between gap-4">
                <h2 id="menu-item-modal-title" className="flex items-center gap-2 text-2xl font-semibold text-brand-on-surface">
                  {item.itemName}
                  {item.itemBestSeller && (
                    <svg
                ...
                <span className="inline-flex flex-shrink-0 rounded-full bg-brand px-4 py-2 text-base font-semibold text-brand-foreground">
                  ${item.itemPrice}
                </span>
              </div>
            ...
            {/* Description */}
            <p className="text-base leading-7 text-brand-on-surface/70">{item.itemDescription}</p>

            {/* Contains / allergens */}
            {(item.allergens?.length ?? 0) > 0 && (
              <div>
                <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-brand-on-surface/50">Contains</h3>
                <ScrollRow>
            ...
            {(item.ingredients ?? []).length > 0 && (
              <div>
                <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-brand-on-surface/50">Ingredients</h3>
                <div className="flex flex-wrap gap-2">
                  {(item.ingredients ?? []).map((ingredient) => (
                    <span key={ingredient} className="rounded-full bg-brand-on-surface/10 px-3 py-1 text-sm text-brand-on-surface/80">
                      {ingredient}
                    </span>
                  ))}
                </div>
            ...
            {alsoTry.length > 0 && (
              <>
                <hr className="border-brand-on-surface/10" />
                <div>
                  <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-brand-on-surface/50">Also Try</h3>
                  <ScrollRow>
                    {alsoTry.map((related) => (
                      <button
                        key={related.itemId}
                      ...
                      >
                        ...
                        <div className="w-24">
                          <p className="truncate text-xs font-semibold leading-snug text-brand-on-surface">{related.itemName}</p>
                          <p className="mt-0.5 text-xs text-brand-on-surface/50">${related.itemPrice}</p>
                        </div>
                      </button>
                    ))}
                  </ScrollRow>

...
export default async function RestaurantMenuPage({ params }: PageProps) {
  ...
  const restaurantName = restaurantNames[restaurantId] || `Restaurant ${restaurantId}`;
    ...
    <main className="min-h-screen bg-zinc-50 px-4 py-8 text-slate-900">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-8">
        <header className="rounded-[2rem] bg-white/95 p-8 text-center shadow-sm shadow-slate-200 backdrop-blur-xl">
          <p className="text-sm uppercase tracking-[0.35em] text-rose-600">Digital menu</p>
                ...
                <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <div className="rounded-full bg-slate-900 px-4 py-2 text-lg font-semibold text-white">
                        ${item.itemPrice}
                      </div>
                    </div>
                  </div>
